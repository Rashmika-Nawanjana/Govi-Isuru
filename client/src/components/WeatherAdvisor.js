...
const WeatherAdvisor = ({ lat, lon, lang, user }) => {
  ...
  if (!weather) return (
    <div className="p-10 text-center">
      ...
      <p className="text-gray-500 dark:text-gray-400 font-medium">{lang === 'si' ? 'කාලගුණය පූරණය වෙමින්...' : 'Detecting local climate...'}</p>
    </div>
  );
        ...
        <div className="p-4 md:p-6">
          <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
            <div className="flex items-center gap-3 md:gap-4">
              ...
              />
              <div>
                <p className="text-4xl md:text-5xl font-black text-gray-800 dark:text-white">{Math.round(weather.main.temp)}°C</p>
                <p className="text-xs md:text-sm text-gray-500 dark:text-gray-400 capitalize font-medium">{weather.weather[0].description}</p>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-2 md:gap-3">
              <div className="bg-orange-50 dark:bg-orange-900/20 px-3 py-2 md:px-4 md:py-3 rounded-xl border border-orange-100 dark:border-gray-700 text-center">
                <Droplets size={16} className="text-blue-500 mx-auto mb-1"/>
                <p className="text-[10px] md:text-xs text-gray-500 dark:text-gray-400">Humidity</p>
                <p className="text-base md:text-lg font-bold text-gray-800 dark:text-white">{weather.main.humidity}%</p>
              </div>
              <div className="bg-blue-50 dark:bg-blue-900/20 px-3 py-2 md:px-4 md:py-3 rounded-xl border border-blue-100 dark:border-gray-700 text-center">
                <Wind size={16} className="text-blue-400 mx-auto mb-1"/>
                <p className="text-[10px] md:text-xs text-gray-500 dark:text-gray-400">Wind</p>
                <p className="text-base md:text-lg font-bold text-gray-800 dark:text-white">{weather.wind.speed} m/s</p>
              </div>
            </div>
          </div>

          <div className="mt-5 p-4 bg-gradient-to-r from-blue-50 to-cyan-50 dark:from-blue-900/20 dark:to-cyan-900/20 rounded-xl border border-blue-100 dark:border-gray-700 flex items-start gap-3">
